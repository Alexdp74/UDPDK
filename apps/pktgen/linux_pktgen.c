...
static void send_body(stats_t *stats)
{
    ...
    while (app_alive) {
        ...
        ret = sendto(sock, (void *)mydata, pktlen, 0,
...
static void recv_body(stats_t *stats)
{
    ...
    while (app_alive) {
        ...
        n = recvfrom(sock, (void *)mydata, MAX_PACKET_LEN, 0, ( struct sockaddr *) &cliaddr, &len);
        if (n > 0) {
            stats->pkts_recv++;
            stats->bytes_recv += n;
            if (hdr_stats) {
...
static int parse_app_args(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, "a:c:f:r:s:l:hd")) != -1) {
        switch (c) {
            case 'a':
                // this is for the static arp file needed by DPDK, not by the app
            case 'c':
